...
`glimpse` is an alternative way of looking at the data.
...
We will next remove the first column and first 3 rows.
...
This table is a bit better but we still do not have column headers. We can get this information from the html of the page. We need to compare `raw_headers` with the headers on the webpage.
...
Looking at the webpage, each type of meat has the two metrics side by side. We need to extract the headers to match our table.
...
We can add the middle headers together in the correct order.
...
We finally paste (concatenate) the headers and set them as the column headers for our original data.
