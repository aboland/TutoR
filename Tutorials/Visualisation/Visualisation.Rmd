...
(A data frame is a rectangular colection of variables. This can be thought of similar to a spreadsheet.)
...
Creating a plot with ggplot uses a set format.  
The data (`<DATA>`) is passed as part of the ggplot function, the type of graph is defined by the function which follows (`<GEOM_FUNCTION>`), and the `<MAPPINGS>` component defines which variables are plotted and can control other parts of the graph like the colour, size, etc...  
