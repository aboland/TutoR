...
Produce a scatterplot with `displ` on the x-axis and `hwy` (highway miles per gallon) on the y-axis. [Exercise 1 Solution]

```{r ex1, exercise=TRUE, include=FALSE}
ggplot()
```
...
Colour the scatterplot created in Exercise 1 using the `class` variable. [Exercise 2 Solution]



```{r ex2, exercise=TRUE, exercise.eval= FALSE, include=FALSE}
ggplot()
```
...
Move the colour inside `aes` to see what happens. [Exercise 3 Solution]

```{r colourScatter_note, exercise= TRUE, exercise.eval= FALSE, include=FALSE}
ggplot(data = mpg) + 
  geom_point(mapping = aes(x = displ, y = cty),  colour = "blue")
```
```{r colourScatter_note-solution, include=FALSE}
ggplot(data = mpg) + 
  geom_point(mapping = aes(x = displ, y = cty,  colour = "blue"))
```
...
Create a barplot of the `cut` variable. [Exercise 4 Solution]


```{r ex3, exercise= TRUE, include=FALSE}
ggplot()
```
...
Create a histogram of price but this time set the `binwidth` parameter to 1000. What effect does this have on the resulting plot? [Exercise 5 Solution]

```{r ex4, exercise= TRUE, include=FALSE}
ggplot()
```
...
Use `coord_flip()` to change the plot. [Exercise 6 Solution]

```{r coord, exercise = TRUE, exercise.eval = TRUE, include=FALSE}
ggplot(data = mpg) + 
  geom_boxplot(aes(x = class, y = hwy))

```  
```{r coord-solution, include=FALSE}
ggplot(data = mpg) + 
  geom_boxplot(aes(x = class, y = hwy)) +
  coord_flip()
```
...
Add a subtitle and caption to the plot below. [Exercise 7 Solution]

```{r labels_title, exercise = TRUE, exercise.eval = TRUE, , exercise.lines=4, include=FALSE}
ggplot(mpg) +
...
```{r labels_title-solution, include=FALSE}
ggplot(mpg) +
  geom_point(aes(displ, hwy, color = class)) +
  labs(title = "Fuel efficiency generally decreases with engine size",
...
```{r colourScatter_note-solution, echo =T}
```
...
```{r labels_title-solution, echo=T}
```
